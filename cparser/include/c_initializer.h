...
class Designator
{
  ...
  public:
    ...
    {
        return std::holds_alternative<std::string>(_designator);
    }
    inline bool is_index()
    {
        return std::holds_alternative<size_t>(_designator);
    }

    inline std::string get_name()
    {
        return std::get<std::string>(_designator);
    }
    inline size_t get_index()
    {
        return std::get<size_t>(_designator);
    }
};
...
{
  public:
    using child_t = std::variant<std::shared_ptr<AggregateUnionObjectMemberTree>,
                                 std::shared_ptr<cparser::ASTNodeVariableType>>;
