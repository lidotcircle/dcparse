...
#define add_binary_rule(nonterm, leftnonterm, rightnonterm, op, astop, assoc, ...)                 \
    parser(                                                                                        \
        ...
        [](auto c, auto ts) {                                                                      \
            ...
            auto ast =                                                                             \
                make_shared<ASTNodeExprBinaryOp>(c, ASTNodeExprBinaryOp::astop, lhsast, rhsast);   \
...
#define add_prefix_unary_rule(lhsnonterm, rhsnonterm, op, astop, assoc)                            \
    parser(                                                                                        \
        ...
        [](auto c, auto ts) {                                                                      \
            ...
            auto ast = make_shared<ASTNodeExprUnaryOp>(c, ASTNodeExprUnaryOp::astop, rhsast);      \
...
#define add_postfix_unary_rule(lhsnonterm, rhsnonterm, op, astop, assoc)                           \
    parser(                                                                                        \
        ...
        [](auto c, auto ts) {                                                                      \
            ...
            auto ast = make_shared<ASTNodeExprUnaryOp>(c, ASTNodeExprUnaryOp::astop, lhsast);      \
...
CParserContext::CParserContext(CParser* parser) : DCParser::DCParserContext(), m_cparser(parser)
{}
...
void CParser::expression_rules()
{
    ...
    parser(
        ...
        {NI(POSTFIX_EXPRESSION),
         PT(LPAREN),
         ParserChar::beOptional(NI(ARGUMENT_EXPRESSION_LIST)),
         PT(RPAREN)},
        [](auto c, auto ts) {
            assert(ts.size() == 4);
    ...
    add_postfix_unary_rule(
        POSTFIX_EXPRESSION, POSTFIX_EXPRESSION, PLUSPLUS, POSFIX_INC, RuleAssocitiveLeft);
    add_postfix_unary_rule(
        POSTFIX_EXPRESSION, POSTFIX_EXPRESSION, MINUSMINUS, POSFIX_DEC, RuleAssocitiveLeft);
    ...
    parser(
        ...
        {PT(LPAREN),
         ...
         PT(LBRACE),
         NI(INITIALIZER_LIST),
         ParserChar::beOptional(PT(COMMA)),
         PT(RBRACE)},
        [](auto c, auto ts) {
            assert(ts.size() == 7);
    ...
    parser(NI(ARGUMENT_EXPRESSION_LIST), {NI(ASSIGNMENT_EXPRESSION)}, [](auto c, auto ts) {
        assert(ts.size() == 1);
    ...
    add_prefix_unary_rule(
        UNARY_EXPRESSION, UNARY_EXPRESSION, PLUSPLUS, PREFIX_INC, RuleAssocitiveRight);
    add_prefix_unary_rule(
        UNARY_EXPRESSION, UNARY_EXPRESSION, MINUSMINUS, PREFIX_DEC, RuleAssocitiveRight);

    add_prefix_unary_rule(UNARY_EXPRESSION, CAST_EXPRESSION, REF, REFERENCE, RuleAssocitiveRight);
    add_prefix_unary_rule(
        UNARY_EXPRESSION, CAST_EXPRESSION, MULTIPLY, DEREFERENCE, RuleAssocitiveRight);
    ...
    add_prefix_unary_rule(
        UNARY_EXPRESSION, CAST_EXPRESSION, BIT_NOT, BITWISE_NOT, RuleAssocitiveRight);
    add_prefix_unary_rule(
        UNARY_EXPRESSION, CAST_EXPRESSION, LOGIC_NOT, LOGICAL_NOT, RuleAssocitiveRight);
    ...
    parser(
        ...
        [](auto c, auto ts) {
            ...
            auto ast = make_shared<ASTNodeExprUnaryOp>(c, ASTNodeExprUnaryOp::SIZEOF, rhsast);
    ...
    parser(
        ...
        [](auto c, auto ts) {
            ...
            auto ast = make_shared<ASTNodeExprSizeof>(c, rhsast);
            return makeNT(UNARY_EXPRESSION, ast);
        },
        RuleAssocitiveLeft);
    ...
    add_binary_rule(MULTIPLICATIVE_EXPRESSION,
    ...
    add_binary_rule(ADDITIVE_EXPRESSION,
                    ADDITIVE_EXPRESSION,
                    MULTIPLICATIVE_EXPRESSION,
                    PLUS,
                    PLUS,
    ...
    add_binary_rule(SHIFT_EXPRESSION,
                    SHIFT_EXPRESSION,
                    ADDITIVE_EXPRESSION,
                    LEFT_SHIFT,
                    LEFT_SHIFT,
    ...
    add_binary_rule(RELATIONAL_EXPRESSION,
    ...
    add_binary_rule(EQUALITY_EXPRESSION,
                    EQUALITY_EXPRESSION,
                    RELATIONAL_EXPRESSION,
                    EQUAL,
                    EQUAL,
    ...
    add_binary_rule(
        AND_EXPRESSION, AND_EXPRESSION, EQUALITY_EXPRESSION, REF, BITWISE_AND, RuleAssocitiveLeft);


    expr_reduce(EXCLUSIVE_OR_EXPRESSION, AND_EXPRESSION);
    add_binary_rule(EXCLUSIVE_OR_EXPRESSION,
    ...
    add_binary_rule(INCLUSIVE_OR_EXPRESSION,
    ...
    add_binary_rule(LOGICAL_AND_EXPRESSION,
    ...
    add_binary_rule(LOGICAL_OR_EXPRESSION,
    ...
    parser(
        NI(CONDITIONAL_EXPRESSION),
        {NI(LOGICAL_OR_EXPRESSION),
         PT(QUESTION),
         NI(EXPRESSION),
         PT(COLON),
         NI(CONDITIONAL_EXPRESSION)},
        ...
        RuleAssocitiveRight,
        nullptr,
        priority_LOGICAL_AND_EXPRESSION_LOGICAL_OR_EXPRESSION);


    expr_reduce(ASSIGNMENT_EXPRESSION, CONDITIONAL_EXPRESSION);
    add_binary_rule(ASSIGNMENT_EXPRESSION,
                    UNARY_EXPRESSION,
                    ...
                    RuleAssocitiveRight,
                    nullptr,
                    priority_UNARY_EXPRESSION_CAST_EXPRESSION);
    add_binary_rule(ASSIGNMENT_EXPRESSION,
                    UNARY_EXPRESSION,
                    ASSIGNMENT_EXPRESSION,
                    MULTIPLY_ASSIGN,
    ...
    parser(NI(EXPRESSION), {NI(ASSIGNMENT_EXPRESSION)}, [](auto c, auto ts) {
        assert(ts.size() == 1);
        get_ast(expr, ASSIGNMENT_EXPRESSION, ASTNodeExpr, 0);
...
mix_type_specifiers(ASTNodeParserContext c,
                    ...
                    shared_ptr<ASTNodeVariableTypePlain> spb)
{
    ...
    if (ntype != variable_basic_type::INT && ntype != variable_basic_type::FLOAT) {
...
void CParser::declaration_rules()
{
    ...
    parser(NI(DECLARATION),
           {NI(DECLARATION_SPECIFIERS),
            ParserChar::beOptional(NI(INIT_DECLARATOR_LIST)),
            PT(SEMICOLON)},
           [](auto c, auto ts) {
               assert(ts.size() == 3);
               auto ctx = c.lock();
               ...
               if (init_decl_list_ast->empty()) {
                   ...
                   auto justfordecl =
                       make_shared<ASTNodeInitDeclarator>(c, id, decl_specast, nullptr);
    ...
    parser(NI(DECLARATION), {NI(STATIC_ASSERT_DECLARATION)}, [](auto c, auto ts) {
        assert(ts.size() == 1);
        ...
        [](auto c, auto ts) {
            ...
            if (ts[0]) {
                ...
                if (dsast->storage_class() != storage_class_t::SC_Default) {
                    get_ctx(ctx, c);
                    auto p = ctx->textinfo();
                    ctx->warn("redefinition of storage class, using first one at [%s]:%s",
                              p->row_col_str(*dsast).c_str(),
                              p->query_string(*dsast).c_str());
                }
    ...
    parser(NI(INIT_DECLARATOR_LIST), {NI(INIT_DECLARATOR)}, [](auto c, auto ts) {
        assert(ts.size() == 1);
...
#define to_storage_specifier(kw, en)                                                               \
    parser(NI(STORAGE_CLASS_SPECIFIER), {KW(kw)}, [](auto c, auto ts) {                            \
        assert(ts.size() == 1);                                                                    \
...
#define to_type_specifier(kw, en, ...)                                                             \
    parser(NI(TYPE_SPECIFIER), {KW(kw)}, [](auto c, auto ts) {                                     \
        assert(ts.size() == 1);                                                                    \
    ...
    parser(
        ...
        //                                                                 extension
        {NI(STRUCT_OR_UNION),
         ParserChar::beOptional(TI(ID)),
         PT(LBRACE),
         ParserChar::beOptional(NI(STRUCT_DECLARATION_LIST)),
         PT(RBRACE)},
        [](auto c, auto ts) {
            assert(ts.size() == 5);
            assert(ts[0]);
            ...
            const auto is_struct =
                dynamic_pointer_cast<struct_pesudo>(struct_or_union->astnode) != nullptr;
            ...
            if (ts[1]) {
                ...
                id = make_shared<TokenID>("#anonymous_struct_union_" +
                                          to_string(++anonymous_struct_union_counter));
            }
    ...
    parser(NI(STRUCT_OR_UNION), {KW(struct)}, [](auto c, auto ts) {
        assert(ts.size() == 1);
        return makeNT(STRUCT_OR_UNION, make_shared<struct_pesudo>(c));
    });

    parser(NI(STRUCT_OR_UNION), {KW(union)}, [](auto c, auto ts) {
        assert(ts.size() == 1);
    ...
    parser(NI(STRUCT_DECLARATION_LIST),
           ...
           [](auto c, auto ts) {
               ...
               get_ast_if_presents(
                   xsdl, STRUCT_DECLARATION_LIST, ASTNodeStructUnionDeclarationList, 0);
               if (xsdlast)
                   sdlast = xsdlast;
    ...
    parser(NI(STRUCT_DECLARATION_LIST),
           ...
           [](auto c, auto ts) {
               ...
               get_ast_if_presents(
                   sdl, STRUCT_DECLARATION_LIST, ASTNodeStructUnionDeclarationList, 0);
               if (sdlast)
                   ast = sdlast;
    ...
    parser(NI(STRUCT_DECLARATOR_LIST), {NI(STRUCT_DECLARATOR)}, [](auto c, auto ts) {
        assert(ts.size() == 1);
        auto sdlast = make_shared<ASTNodeStructUnionDeclarationList>(c);
    ...
    parser(
        ...
        {KW(enum),
         ParserChar::beOptional(TI(ID)),
         PT(LBRACE),
         ParserChar::beOptional(NI(ENUMERATOR_LIST)),
         ParserChar::beOptional(PT(COMMA)),
         PT(RBRACE)},
        [](auto c, auto ts) {
            assert(ts.size() == 6);
            ...
            if (ts[1]) {
                ...
                id = make_shared<TokenID>("anonymous_enum_" + to_string(++anonymous_enum_count));
            }
    ...
    parser(NI(ENUMERATOR_LIST), {NI(ENUMERATOR)}, [](auto c, auto ts) {
        assert(ts.size() == 1);
    ...
    parser(NI(ENUMERATION_CONSTANT), {TI(ID)}, [](auto c, auto ts) {
        assert(ts.size() == 1);
        auto id = dynamic_pointer_cast<TokenID>(ts[0]);
...
#define add_to_qualifier(kw)                                                                       \
    parser(NI(TYPE_QUALIFIER), {KW(kw)}, [](auto c, auto ts) {                                     \
        assert(ts.size() == 1);                                                                    \
        auto ast = make_shared<ASTNodeVariableTypeDummy>(c);                                       \
        ast->kw##_ref() = true;                                                                    \
...
#define add_to_function_specifier(kw, kw_ref)                                                      \
    parser(NI(FUNCTION_SPECIFIER), {KW(kw)}, [](auto c, auto ts) {                                 \
        assert(ts.size() == 1);                                                                    \
        auto ast = make_shared<ASTNodeVariableTypeDummy>(c);                                       \
        ast->kw_ref() = true;                                                                      \
    ...
    parser(NI(DIRECT_DECLARATOR), {TI(ID)}, [](auto c, auto ts) {
        assert(ts.size() == 1);
        auto id = dynamic_pointer_cast<TokenID>(ts[0]);
    ...
    parser(
        ...
        {NI(DIRECT_DECLARATOR),
         PT(LBRACKET),
         ParserChar::beOptional(NI(TYPE_QUALIFIER_LIST)),
         ParserChar::beOptional(NI(ASSIGNMENT_EXPRESSION)),
         PT(RBRACKET)},
        [](auto c, auto ts) {
            assert(ts.size() == 5);
    ...
    parser(
        ...
        {NI(DIRECT_DECLARATOR),
         PT(LPAREN),
         ParserChar::beOptional(NI(IDENTIFIER_LIST)),
         PT(RPAREN)},
        [](auto c, auto ts) {
            assert(ts.size() == 4);
            get_ast(dd, DIRECT_DECLARATOR, ASTNodeInitDeclarator, 0);
    ...
    parser(
        NI(POINTER),
        {ParserChar::beOptional(NI(POINTER)),
         PT(MULTIPLY),
         ParserChar::beOptional(NI(TYPE_QUALIFIER_LIST))},
        [](auto c, auto ts) {
            assert(ts.size() == 3);
    ...
    parser(NI(PARAMETER_LIST), {NI(PARAMETER_DECLARATION)}, [](auto c, auto ts) {
        assert(ts.size() == 1);
    ...
    parser(
        ...
        [](auto c, auto ts) {
            ...
            auto ast =
                make_shared<ASTNodeParameterDeclaration>(c, dast->get_id(), dast->get_type());
    ...
    parser(NI(IDENTIFIER_LIST), {TI(ID)}, [](auto c, auto ts) {
        assert(ts.size() == 1);
    ...
    parser(NI(IDENTIFIER_LIST), {NI(IDENTIFIER_LIST), PT(COMMA), TI(ID)}, [](auto c, auto ts) {
        assert(ts.size() == 3);
    ...
    parser(
        NI(DIRECT_ABSTRACT_DECLARATOR),
        {ParserChar::beOptional(NI(DIRECT_ABSTRACT_DECLARATOR)),
         PT(LBRACKET),
         ParserChar::beOptional(NI(TYPE_QUALIFIER_LIST)),
         ParserChar::beOptional(NI(ASSIGNMENT_EXPRESSION)),
         PT(RBRACKET)},
        [](auto c, auto ts) {
            ...
            if (!ddast)
                ddast = make_shared<ASTNodeInitDeclarator>(c, nullptr, nullptr, nullptr);
            ddast->set_leaf_type(array);
    ...
    parser(
        ...
        {ParserChar::beOptional(NI(DIRECT_ABSTRACT_DECLARATOR)),
         PT(LPAREN),
         ParserChar::beOptional(NI(PARAMETER_TYPE_LIST)),
         PT(RPAREN)},
        [](auto c, auto ts) {
            assert(ts.size() == 4);
            get_ast_if_presents(dd, DIRECT_ABSTRACT_DECLARATOR, ASTNodeInitDeclarator, 0);
    ...
    parser(NI(INITIALIZER), {NI(ASSIGNMENT_EXPRESSION)}, [](auto c, auto ts) {
        assert(ts.size() == 1);
        get_ast(expr, ASSIGNMENT_EXPRESSION, ASTNodeExpr, 0);
    ...
    parser(NI(INITIALIZER),
           {PT(LBRACE),
            ParserChar::beOptional(NI(INITIALIZER_LIST)),
            ParserChar::beOptional(PT(COMMA)),
            PT(RBRACE)},
           [](auto c, auto ts) {
               assert(ts.size() == 4);
    ...
    parser(NI(DESIGNATION), {NI(DESIGNATOR_LIST), PT(ASSIGN)}, [](auto c, auto ts) {
        assert(ts.size() == 2);
    ...
    parser(
        NI(DESIGNATOR), {PT(LBRACKET), NI(CONSTANT_EXPRESSION), PT(RBRACKET)}, [](auto c, auto ts) {
            assert(ts.size() == 3);
    ...
    parser(NI(DESIGNATOR), {PT(DOT), TI(ID)}, [](auto c, auto ts) {
        assert(ts.size() == 2);
        auto id = dynamic_pointer_cast<TokenID>(ts[1]);
        assert(id);
...
#define to_statement(xt)                                                                           \
    parser(NI(STATEMENT), {NI(xt)}, [](auto c, auto ts) {                                          \
        assert(ts.size() == 1);                                                                    \
    ...
    parser(NI(LABELED_STATEMENT), {TI(ID), PT(COLON), NI(STATEMENT)}, [](auto c, auto ts) {
        assert(ts.size() == 3);
        auto id = dynamic_pointer_cast<TokenID>(ts[0]);
        assert(id);
    ...
    parser(NI(LABELED_STATEMENT), {KW(default), PT(COLON), NI(STATEMENT)}, [](auto c, auto ts) {
        assert(ts.size() == 3);
        get_ast(st, STATEMENT, ASTNodeStat, 2);
    ...
    parser(NI(BLOCK_ITEM), {NI(DECLARATION)}, [](auto c, auto ts) {
        assert(ts.size() == 1);
        get_ast(d, DECLARATION, ASTNodeBlockItem, 0);
        return makeNT(BLOCK_ITEM, dast);
    });

    parser(NI(BLOCK_ITEM), {NI(STATEMENT)}, [](auto c, auto ts) {
        assert(ts.size() == 1);
    ...
    parser(NI(JUMP_STATEMENT), {KW(goto), TI(ID), PT(SEMICOLON)}, [](auto c, auto ts) {
        assert(ts.size() == 3);
        auto id = dynamic_pointer_cast<TokenID>(ts[1]);
        assert(id);
    ...
    parser(NI(JUMP_STATEMENT), {KW(continue), PT(SEMICOLON)}, [](auto c, auto ts) {
        assert(ts.size() == 2);
        auto ast = make_shared<ASTNodeStatContinue>(c);
        return makeNT(JUMP_STATEMENT, ast);
    });

    parser(NI(JUMP_STATEMENT), {KW(break), PT(SEMICOLON)}, [](auto c, auto ts) {
        assert(ts.size() == 2);
...
void CParser::external_definitions()
{
    ...
    parser(NI(EXTERNAL_DECLARATION), {NI(FUNCTION_DEFINITION)}, [](auto c, auto ts) {
        assert(ts.size() == 1);
        get_ast(cur, FUNCTION_DEFINITION, ASTNodeFunctionDefinition, 0);
        return makeNT(EXTERNAL_DECLARATION, curast);
    });

    parser(NI(EXTERNAL_DECLARATION), {NI(DECLARATION)}, [](auto c, auto ts) {
        assert(ts.size() == 1);
    ...
    parser(
        ...
        {NI(DECLARATION_SPECIFIERS),
         NI(DECLARATOR),
         ParserChar::beOptional(NI(DECLARATION_LIST)),
         NI(COMPOUND_STATEMENT)},
        [](auto c, auto ts) {
            assert(ts.size() == 4);
            ...
            if (decltxast != nullptr) {
                ...
                for (auto decl : *decltxast) {
                    ...
                    if (!declx->get_id()) {
                        throw CErrorParser(
                            "old style function parameter declaration without ID, at " +
                            position_info_of(declx, c));
                    }
                    if (declx->get_initializer()) {
                        throw CErrorParser(
                            "old style function parameter can't be initialized, at " +
                            position_info_of(declx, c));
                    }
                ...
                for (auto decl : *functype->parameter_declaration_list()) {
                    if (!decl->get_id() || decl->get_type()) {
                        throw CErrorParser(
                            "old style function parameter need identifier list in parenthese, at " +
