...
TEST(should_accpet, CAST)
{
    ...
    vector<string> test_cases = {
        ...
        "void test2() {\n"
        "    struct s1 { struct s2 { int a; int b; struct s5 { float a; float b; } c; } v2; struct "
        "s3 { void* a; int   b; } v3; };\n"
        "    struct s1 s1; struct s2 s2; struct s3 s3;\n"
        "    struct s1 val[] = { 1, 2, { }, { }, s2, s3,  { } };\n"
        ...
        "void test3()\n"
        "{\n"
        "   struct s1 { struct s2 {} a1; int b1; struct s3 { struct s4 { } n1; int a, b, c; } a2; "
        "int* c1, c2; };\n"
...
#define EXPECT_WARNING_LIST                                                                        \
    EENTRY("struct ax {}; _Static_assert(sizeof(struct ax) == 0, \"\");",                          \
           EmptyStructUnionDefinition)                                                             \
    ...
    const auto eval_code = [&](string code) {
        ...
        auto reporter = make_shared<SemanticReporter>();
        ASSERT_NO_THROW(
            for (auto c : code) { parser.feed(c); }

            auto tunit = parser.end();
            ASSERT_NE(tunit, nullptr);
            try { tunit->check_constraints(reporter); } catch (CErrorStaticAssert&){})
            << code;
