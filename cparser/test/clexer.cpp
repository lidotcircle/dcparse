...
{
    EXPECT_NO_THROW(cparser::CLexerUTF8 lexer;);
}
...
TEST(should_accept, CLexerBasic)
{
    ...
    vector<pair<string, size_t>> test_cases = {
        ...
        // TODO out of range constant, report error
        {"01e+99 22E1 33e-3 02e22 02e22f 02e77F", 6},
        {"0.33 0.12 5. .8", 4},
    };
    ...
    for (auto ts : test_cases) {
        ...
        EXPECT_EQ(n, tokens.size()) << t << " " << n << endl << tokens2str(tokens);
...
#undef K_ENTRY
    ...
    for (auto ts : test_cases) {
        ...
        ASSERT_EQ(1, tokens.size()) << t << endl << tokens2str(tokens);

        auto token = tokens[0];
        EXPECT_EQ(id, token->charid()) << t << endl << tokens2str(tokens);
...
TEST(shoud_reject, CLexerBasic)
{
    ...
    for (auto ts : test_cases) {
        EXPECT_ANY_THROW(vector<shared_ptr<LexerToken>> tokens; for (auto c : ts) {
            for (auto x : lexer.feed(c))
                tokens.push_back(x);
        } for (auto x : lexer.end()) tokens.push_back(x);

                         EXPECT_TRUE(false) << tokens.size() << "  " << tokens2str(tokens););
