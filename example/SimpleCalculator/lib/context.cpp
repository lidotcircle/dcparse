...
class ScopeContext
{
  ...
  public:
    ScopeContext() = delete;
    ScopeContext(shared_ptr<SCalcFunctionState> func, shared_ptr<SCalcLoopStatState> loop)
        : _function(func), _loop(loop)
    {}

    shared_ptr<SCalcFunctionState> function()
    {
        return this->_function;
    }
    const shared_ptr<SCalcFunctionState> function() const
    {
        return this->_function;
    }
    ...
    const shared_ptr<SCalcLoopStatState> loop() const
    {
        return this->_loop;
    }
    void continueit()
...
SCalcContext::SCalcContext() : _stack({make_shared<ScopeContext>(nullptr, nullptr)})
{
...
class SCalcBuiltinFunction : public SCalcFunction
{
  ...
  public:
    SCalcBuiltinFunction(std::function<double(const vector<double>&)> func) : _func(func)
    {}
...
{
#define CMENTRY(func)                                                                              \
    this->_funcs[#func] = make_shared<SCalcBuiltinFunction>([](const vector<double>& params) {     \
        if (params.size() != 1)                                                                    \
...
#undef CMENTRY
    ...
    this->_funcs["fmod"] = make_shared<SCalcBuiltinFunction>([](const vector<double>& params) {
        if (params.size() != 2)
            throw SCalcBadFunctionArgumentCount();
        return ::fmod(params[0], params[1]);
    });

    this->_funcs["sum"] = make_shared<SCalcBuiltinFunction>([](const vector<double>& params) {
        double ret = 0;
        for (auto& p : params)
            ret += p;
        return ret;
    ...
    this->_funcs["is_int"] = make_shared<SCalcBuiltinFunction>([](const vector<double>& params) {
        if (params.size() != 1)
            throw SCalcBadFunctionArgumentCount();
        return params[0] == floor(params[0]);
    });

    this->_funcs["assert"] = make_shared<SCalcBuiltinFunction>([](const vector<double>& params) {
        if (params.size() != 1)
            throw SCalcBadFunctionArgumentCount();
        if (params[0] == 0)
    ...
    this->_funcs["assert_eq"] = make_shared<SCalcBuiltinFunction>([](const vector<double>& params) {
        if (params.size() != 2)
            throw SCalcBadFunctionArgumentCount();
    ...
    this->_funcs["assert_ne"] = make_shared<SCalcBuiltinFunction>([](const vector<double>& params) {
        if (params.size() != 2)
            throw SCalcBadFunctionArgumentCount();
        if (params[0] == params[1])
            throw SCalcAssertionFailed();
    ...
    this->_funcs["assert_ge"] = make_shared<SCalcBuiltinFunction>([](const vector<double>& params) {
        if (params.size() != 2)
            throw SCalcBadFunctionArgumentCount();
        if (params[0] < params[1])
            throw SCalcAssertionFailed();
    ...
    this->_funcs["assert_gt"] = make_shared<SCalcBuiltinFunction>([](const vector<double>& params) {
        if (params.size() != 2)
            throw SCalcBadFunctionArgumentCount();
        if (params[0] <= params[1])
            throw SCalcAssertionFailed();
    ...
    this->_funcs["assert_le"] = make_shared<SCalcBuiltinFunction>([](const vector<double>& params) {
        if (params.size() != 2)
            throw SCalcBadFunctionArgumentCount();
        if (params[0] > params[1])
            throw SCalcAssertionFailed();
    ...
    this->_funcs["assert_lt"] = make_shared<SCalcBuiltinFunction>([](const vector<double>& params) {
        if (params.size() != 2)
            throw SCalcBadFunctionArgumentCount();
        if (params[0] >= params[1])
            throw SCalcAssertionFailed();
    ...
    this->_funcs["print"] = make_shared<SCalcBuiltinFunction>([](const vector<double>& params) {
