...
void ASTNodeVisitorLLVMGen::visitExpr(const ASTNodeExpr& expr)
{
    if (dynamic_cast<const IDExpr*>(&expr)) {
        ...
        case BinaryOperatorExpr::ASSIGNMENT:
        {
            ...
            if (!id) throw SCalcError("expected an id");
            auto v = this->lookup_var(id->id());
            if (!v) {
                if (m_globalscope) { 
                    auto gv = new llvm::GlobalVariable(llvm::Type::getDoubleTy(*m_context), false, llvm::GlobalVariable::PrivateLinkage, nullptr, id->id());
                    m_module->insertGlobalVariable(gv);
                    m_namedValues.back().insert({id->id(), gv});
                    v = gv;
                } else {
                    auto addr = m_builder->CreateAlloca(llvm::Type::getDoubleTy(*m_context), nullptr, "local");
        ...
        case BinaryOperatorExpr::EQUAL:
            ...
            m_prevValue = m_builder->CreateCast(llvm::Instruction::CastOps::UIToFP, m_prevValue, llvm::Type::getDoubleTy(*m_context), "cast2fp_r");
            break;
        ...
        {
            auto v = m_builder->CreateFAdd(oldvalue, llvm::ConstantFP::get(*m_context, llvm::APFloat(1.0)), "inc1");
            m_builder->CreateStore(v, addr);
            m_prevValue = v;
        } break;
        case UnaryOperatorExpr::PRE_DEC:
        {
            auto v = m_builder->CreateFAdd(oldvalue, llvm::ConstantFP::get(*m_context, llvm::APFloat(-1.0)), "dec1");
            m_builder->CreateStore(v, addr);
            m_prevValue = v;
        } break;
        case UnaryOperatorExpr::POS_INC:
        {
            auto v = m_builder->CreateFAdd(oldvalue, llvm::ConstantFP::get(*m_context, llvm::APFloat(1.0)), "inc1x");
            m_builder->CreateStore(v, addr);
            m_prevValue = oldvalue;
        } break;
        case UnaryOperatorExpr::POS_DEC:
        {
            auto v = m_builder->CreateFAdd(oldvalue, llvm::ConstantFP::get(*m_context, llvm::APFloat(-1.0)), "dec1x");
            m_builder->CreateStore(v, addr);
            m_prevValue = oldvalue;
        } break;
        }
        ...
        if (!id) throw SCalcError("invalid function call");
        const auto funcname = id->id();
        llvm::Function* f = m_module->getFunction(funcname);
        if (!f) throw SCalcError("undefined function '" + funcname + "'");
        if (f->arg_size() != e.parameters()->size()) throw SCalcError("incorrect number of parameters");
...
{
    for (auto& stat: statlist) stat->accept(*this);
}

void ASTNodeVisitorLLVMGen::visitForStat(const ASTNodeExprList* pre, const ASTNodeExpr* cond, const ASTNodeExprList* post, const ASTNodeStat& stat)
{
    if (pre) {
        for (auto& e: *pre)
            e->accept(*this);
    ...
    if (cond) {
        cond->accept(*this);
        cv = m_builder->CreateFCmpONE(m_prevValue, llvm::ConstantFP::get(*m_context, llvm::APFloat(0.0)), "forcond");
    } else {
...
void ASTNodeVisitorLLVMGen::visitIfStat(const ASTNodeExpr& cond, const ASTNodeStat& then, const ASTNodeStat* else_)
{
    cond.accept(*this);
    auto cmp = m_builder->CreateFCmpONE(m_prevValue, llvm::ConstantFP::get(*m_context, llvm::APFloat(0.0)), "ifcond");
    auto func = m_builder->GetInsertBlock()->getParent();
...
static llvm::AllocaInst *CreateEntryBlockAlloca(llvm::Function *TheFunction,
                                                llvm::StringRef VarName, llvm::LLVMContext& context)
{
...
void ASTNodeVisitorLLVMGen::visitFuncDef(const std::string& funcname, const std::vector<std::string>& args, const ASTNodeBlockStat& block)
{
    std::vector<llvm::Type*> argstype(args.size(), llvm::Type::getDoubleTy(*m_context));
    llvm::FunctionType* ft = llvm::FunctionType::get(llvm::Type::getDoubleTy(*m_context), argstype, false);
    llvm::Function* f = llvm::Function::Create(ft, llvm::Function::LinkageTypes::InternalLinkage, funcname, m_module.get());
    size_t i=0;
    m_namedValues.emplace_back();
...
void ASTNodeVisitorLLVMGen::visitFuncDecl(const std::string& funcname, const std::vector<std::string>& args)
{
    std::vector<llvm::Type*> argstype(args.size(), llvm::Type::getDoubleTy(*m_context));
    llvm::FunctionType* ft = llvm::FunctionType::get(llvm::Type::getDoubleTy(*m_context), argstype, false);
    llvm::Function* f = llvm::Function::Create(ft, llvm::Function::LinkageTypes::ExternalLinkage, funcname, m_module.get());
}

void ASTNodeVisitorLLVMGen::visitCalcUnit(const std::vector<UnitItem>& items)
{
    std::vector<llvm::Type*> argstype;
    llvm::FunctionType* ft = llvm::FunctionType::get(llvm::Type::getInt32Ty(*m_context), argstype, false);
    llvm::Function* f = llvm::Function::Create(ft, llvm::Function::LinkageTypes::ExternalLinkage, "main", m_module.get());
    llvm::BasicBlock* gblock = llvm::BasicBlock::Create(*m_context, "main_entry", f);
    for (auto& item: items) {
