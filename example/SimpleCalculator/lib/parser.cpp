    ...
    parser(                                                                                        \
        ...
        [](auto c, auto ts) {                                                                      \
            ...
            auto ast = make_shared<BinaryOperatorExpr>(                                            \
                c, BinaryOperatorExpr::BinaryOperatorType::op, astleft, astright);                 \
    ...
    parser(                                                                                        \
        ...
        [](auto c, auto ts) {                                                                      \
            ...
            auto ast = make_shared<UnaryOperatorExpr>(                                             \
                c, UnaryOperatorExpr::UnaryOperatorType::type, astexpr);                           \
            return make_shared<NonTermExpr>(ast);                                                  \
        },                                                                                         \
        assoc);
...
void CalcParser::expression_rules()
{
    ...
    parser(NI(Expr),
           {NI(Expr), TI(LPAREN), ParserChar::beOptional(NI(ExprList)), TI(RPAREN)},
           [](auto c, auto ts) {
               assert(ts.size() == 4);
    ...
    parser(NI(Expr), {TI(LPAREN), NI(Expr), TI(RPAREN)}, [](auto c, auto ts) {
        assert(ts.size() == 3);
    ...
    parser(NI(Expr), {TI(ID)}, [](auto c, auto ts) {
        assert(ts.size() == 1);
    ...
    parser(NI(Expr), {TI(NUMBER)}, [](auto c, auto ts) {
        assert(ts.size() == 1);
    ...
    parser(NI(ExprList), {NI(Expr)}, [](auto c, auto ts) {
        assert(ts.size() == 1);
    ...
    parser(NI(ExprList), {NI(ExprList), TI(COMMA), NI(Expr)}, [](auto c, auto ts) {
        assert(ts.size() == 3);
...
void CalcParser::statement_rules()
{
    ...
    parser(NI(ExprStatement), {NI(ExprList), TI(SEMICOLON)}, [](auto c, auto ts) {
        assert(ts.size() == 2);
        pnonterm(ExprList, ASTNodeExprList, 0, exprlist);
    ...
    parser(NI(ExprStatement), {NI(ExprList), TI(NEWLINE)}, [](auto c, auto ts) {
        assert(ts.size() == 2);
        pnonterm(ExprList, ASTNodeExprList, 0, exprlist);
    ...
    parser(NI(ExprStatement), {TI(SEMICOLON)}, [](auto c, auto ts) {
        assert(ts.size() == 1);
    ...
    parser(NI(StatementList), {NI(Statement)}, [](auto c, auto ts) {
        assert(ts.size() == 1);
    ...
    parser(NI(StatementList), {NI(StatementList), NI(Statement)}, [](auto c, auto ts) {
        assert(ts.size() == 2);
    ...
    parser(NI(BlockStatement),
           {TI(LBRACE), ParserChar::beOptional(NI(StatementList)), TI(RBRACE)},
           [](auto c, auto ts) {
               assert(ts.size() == 3);
    ...
    parser(
        NI(IfStatement),
        {TI(IF), TI(LPAREN), NI(Expr), TI(RPAREN), NI(Statement), TI(ELSE), NI(Statement)},
        [](auto c, auto ts) {
            assert(ts.size() == 7);
    ...
    parser(
        NI(IfStatement),
        {TI(IF), TI(LPAREN), NI(Expr), TI(RPAREN), NI(Statement)},
        [](auto c, auto ts) {
            assert(ts.size() == 5);
            pnonterm(Expr, ASTNodeExpr, 2, cond);
            pnonterm(Statement, ASTNodeStat, 4, truestat);
    ...
    parser(NI(WhileStatement),
           {TI(WHILE), TI(LPAREN), NI(Expr), TI(RPAREN), NI(Statement)},
           [](auto c, auto ts) {
               assert(ts.size() == 5);
               pnonterm(Expr, ASTNodeExpr, 2, cond);
    ...
    parser(NI(ReturnStatement), {TI(RETURN), NI(Expr), TI(SEMICOLON)}, [](auto c, auto ts) {
        assert(ts.size() == 3);
        pnonterm(Expr, ASTNodeExpr, 1, expr);
    ...
    parser(NI(ReturnStatement), {TI(RETURN), NI(Expr), TI(NEWLINE)}, [](auto c, auto ts) {
        assert(ts.size() == 3);
        pnonterm(Expr, ASTNodeExpr, 1, expr);
        auto ast = make_shared<ASTNodeReturnStat>(c, exprast);
...
#define to_statement(type)                                                                         \
    parser(NI(Statement), {NI(type)}, [](auto c, auto ts) {                                        \
        assert(ts.size() == 1);                                                                    \
...
void CalcParser::function_rules()
{
    ...
    parser(NI(FunctionDef),
           {TI(FUNCTION),
            NI(Expr),
            TI(LPAREN),
            ParserChar::beOptional(NI(ArgList)),
            TI(RPAREN),
            NI(BlockStatement)},
           [](auto c, auto ts) {
               assert(ts.size() == 6);
    ...
    parser(NI(ArgList), {TI(ID)}, [](auto c, auto ts) {
        assert(ts.size() == 1);
    ...
    parser(NI(ArgList), {NI(ArgList), TI(COMMA), TI(ID)}, [](auto c, auto ts) {
        assert(ts.size() == 3);
...
void CalcParser::calcunit_rules()
{
    ...
    parser(NI(CalcUnit), {NI(Statement)}, [](auto c, auto ts) {
        assert(ts.size() == 1);
        pnonterm(Statement, ASTNodeStat, 0, stat);
    ...
    parser(NI(CalcUnit), {NI(FunctionDef)}, [](auto c, auto ts) {
        assert(ts.size() == 1);
    ...
    parser(NI(CalcUnit), {NI(CalcUnit), NI(Statement)}, [](auto c, auto ts) {
        assert(ts.size() == 2);
    ...
    parser(NI(CalcUnit), {NI(CalcUnit), NI(FunctionDef)}, [](auto c, auto ts) {
        assert(ts.size() == 2);
        pnonterm(CalcUnit, ASTNodeCalcUnit, 0, unit);
    ...
    parser(NI(CalcUnit), {NI(FunctionDecl)}, [](auto c, auto ts) {
        assert(ts.size() == 1);
    ...
    parser(NI(CalcUnit), {NI(CalcUnit), NI(FunctionDecl)}, [](auto c, auto ts) {
        assert(ts.size() == 2);
        pnonterm(CalcUnit, ASTNodeCalcUnit, 0, unit);
