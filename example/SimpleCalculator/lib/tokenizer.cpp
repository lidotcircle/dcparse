...
CalcLexer::CalcLexer()
{
    ...
    lexer(std::make_unique<LexerRuleRegex<int>>(
        s2u("//[^\n]*\n"), [](auto str, auto info) { return nullptr; }, false, true));

    lexer.dec_priority_major();

#define TENTRY(cn, reg)                                                                            \
    lexer(std::make_unique<LexerRuleRegex<int>>(                                                   \
        s2u(reg), [](auto str, auto info) { return std::make_shared<Token##cn>(info); }));
    KEYWORD_TOKEN
#undef TENTRY
    ...
    lexer(std::make_unique<LexerRuleRegex<int>>(
        ...
        [](auto last) {
            ...
            static const auto possible_prev =
                calcparser.prev_possible_token_of(CharID<TokenNUMBER>());
    ...
    // space, tab, newline
    lexer(std::make_unique<LexerRuleRegex<int>>(s2u("( |\t|\r)+"),
