...
class Regex2BasicConvertor
{
  private:
    ...
    std::vector<regex_char> get_last_node(size_t last_group_pos)
    {
        ...
        if (last_group_pos == npos) {
            ...
            assert(this->__result.back().get() == traits::RPAREN ||
                   this->__result.back().get() == traits::RBRACKET);
            std::vector<regex_char> ans;
            std::copy(this->__result.begin() + last_group_pos,
                      this->__result.end(),
    ...
    bool handle_brace_mode(regex_char _c)
    {
        ...
        if (c == traits::COMMA) {
            ...
            for (size_t i = 0; i < this->m_brace_count_low; i++) {
                this->__result.insert(
                    this->__result.end(), this->__copy_content.begin(), this->__copy_content.end());
            }
            ...
            if (this->m_brace_got_comma) {
                ...
                if (this->m_brace_count_up == brace_infinity) {
                    this->__result.insert(this->__result.end(),
                                          this->__copy_content.begin(),
                                          this->__copy_content.end());
                    ...
                    std::vector<regex_char> nrparen(opt_count,
                                                    regex_char::unescape(traits::RPAREN));
  ...
  public:
    Regex2BasicConvertor()
        : endding(false), m_bracket_mode(false), m_last_group_pos(npos), m_in_brace_mode(false)
    {}
