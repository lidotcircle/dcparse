...
protected:
    ...
    void SetUp() override {
        ...
        parser ( NI(Expr),
                { TI(LPAREN), NI(Type), TI(RPAREN), NI(Expr) },
                [](auto, auto ts) {
                    assert(ts.size() == 4);
                    return make_shared<NonTermExpr>(cs2s(ts));
                } );
        ...
        parser ( NI(Expr),
                { TI(LPAREN), NI(Expr), TI(RPAREN), NI(Expr) },
                [](auto, auto ts) {
                    assert(ts.size() == 4);
                    return make_shared<NonTermExpr>(cs2s(ts));
                } );
...
TEST_F(TypedefParserTest, beyongCF) {
    vector<std::pair<vector<dctoken_t>,string>> test_cases = {
        { { MD("id"), MK(SEMICOLON), }, "e-id;" },
        { { MD("int"), MK(SEMICOLON), }, "e-int;" },
        { { MK(TYPEDEF), MD("int"), MK(SEMICOLON), MK(LPAREN), MD("int"), MK(RPAREN), MD("hello"), MK(SEMICOLON) }, "dint;(t-int)e-hello;" },
        { { MK(TYPEDEF), MD("long"), MK(SEMICOLON), MD("long"), MD("hello"), MK(SEMICOLON) }, "dlong;t-longhello;" },
        { { MK(TYPEDEF), MD("float"), MK(SEMICOLON), MD("kv"), MD("float"), MK(SEMICOLON) }, "dfloat;e-kvfloat;" },
        { { MK(TYPEDEF), MD("double"), MK(SEMICOLON), MD("kv"), MD("double"), MD("xm"), MK(SEMICOLON) }, "ddouble;e-kvdoublexm;" },
        { { MK(TYPEDEF), MD("complex"), MK(SEMICOLON), MD("kv"), MD("xm"), MD("complex"), MK(SEMICOLON) }, "dcomplex;e-kvxmcomplex;" },
    };
